...
import { exportDraft } from './services/exportService';
...
const App: React.FC = () => {
    ...
    const saveTimeout = setTimeout(async () => {
      ...
      try {
        ...
        await db.drafts.update(currentDraftId, {
          ...
          updatedAt: Date.now(),
          suggestions, // Persist suggestions
          comments     // Persist comments
        });
  ...
  }, [content, tone, title, currentDraftId, suggestions, comments]);
  ...
  const handleExportClick = async (format: 'txt' | 'docx' | 'pdf' | 'md' | 'copy' | 'epub') => {
    setShowExportMenu(false);
    ...
    setIsExporting(true);
    try {
      await exportDraft(format, title, content, comments, setExportStatus);
    } catch (e) {
            ...
            {showExportMenu && (
              <div className="absolute top-full right-0 mt-3 w-72 glass rounded-[1.5rem] shadow-2xl py-2 z-50 overflow-hidden border border-white/40 animate-in fade-in slide-in-from-top-2">
                {EXPORT_OPTIONS.map(opt => (
                  <button 
                    key={opt.id} 
                    onClick={() => handleExportClick(opt.id as any)} 
